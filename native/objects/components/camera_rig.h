...
public:
    ...
    glm::quat getPosePrediction(long long time_point);
    glm::quat getPosePrediction(long long time_point,
            const RotationSensorData& rotationSensorData);
    void setPoseState(float* pose_state);

    void setHeadTransform(Transform *transform);
    Transform* getHeadTransform() const; // for rotation/k-sensor
    glm::vec3 getLookAt() const;

    const glm::quat& getComplementaryRotation() const;
    void setComplementaryRotation(const glm::quat& q);
...
private:
    ...
    Transform *head_transform_ = nullptr;
