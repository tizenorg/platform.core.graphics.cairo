...
public:
    ...
    static void renderCamera(Scene* scene, Camera* camera,
            RenderTexture* render_texture, int viewportX, int viewportY,
    ...
    static void renderDistortionOneEye(RenderTexture* render_texture,
            Distorter* distorter, DistortionGrid* distortion_grid,
            bool leftEye);

    static void renderDistortionTwoEyes(RenderTexture* left_render_texture,
            RenderTexture* right_render_texture, Distorter* distorter,
            DistortionGrid* distortion_grid);

    static void frontBufferRenderDistortion(RenderTexture* render_texture,
            Distorter* distorter, DistortionGrid* distortion_grid,
            bool leftEye);

    static void setTimewarpData(Distorter* distorter, float* pose_predicted,
            float* pose_draw, bool use_timewarp);

    static void initRendererGL();

    static void initFrontBufferScreen();

    static void flushFramebuffer(int buffer_idx);

    static bool finishFramebuffer(int buffer_idx, bool force);

    static void invalidateFramebuffer(int color_buffer);

    static bool waitSync();

    static void readRenderResult(RenderTexture* render_texture,
            uint8_t *readback_buffer);

    static void cull(Scene *scene, Camera *camera,
            ShaderManager* shader_manager);

    static void setChromaticAberrationMode(Distorter* distorter,
