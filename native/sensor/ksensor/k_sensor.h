...
#ifdef DEBUG_SENSOR_RECORDER
static const int FloatDataSize = 14;
typedef float FloatData[FloatDataSize];
#endif
...
#ifdef DEBUG_SENSOR_RECORDER
    bool toggleRecording(const FloatData data);
    bool toggleReplaying(FloatData data);
#endif
...
private:
    ...
    bool first_ = true;
    int step_ = 0;
    unsigned int first_real_time_delta_ = 0.0f;
    uint16_t last_timestamp_ = 0;
    uint32_t full_timestamp_ = 0;
    uint8_t last_sample_count_ = 0;
    long long latest_time_ = 0;
    ...
    JavaVM* jvm_ = nullptr;  //
    jclass gvrActivityClass_ = nullptr; //
#endif

#ifdef DEBUG_SENSOR_RECORDER
    std::fstream replayFile_;
