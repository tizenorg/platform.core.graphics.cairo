...
void KSensor::readerThreadFunc() {
    ...
    if (nullptr != activity_ && nullptr !=  gvrActivityClass_) {
        JNIEnv *env;
...
#ifdef DEBUG_SENSOR_RECORDER
        if (recording_) {
            replayFile_ << q_.w << " " << q_.x << " " << q_.y << " " << q.z << " "
                    << last_corrected_gyro_.x << " " << last_corrected_gyro_.y << " " << last_corrected_gyro_.z << " "
                    << last_acceleration_.x << " " << last_acceleration_.y << " " << last_acceleration_.z << std::endl;
        } else if (replaying_) {
            glm::quat q;
            glm::vec3 g;
            glm::vec3 a;
            replayFile_ >> q.w >> q.x >> q.y >> q.z >> g.x >> g.y >> g.z >> a.x >> a.y >> a.z;
            if (!replayFile_.eof()) {
                q_ = q;
                last_corrected_gyro_ = g;
            }
        }
#endif
    }
...
KSensor::KSensor() :
        gyroFilter_(KGyroNoiseFilterCapacity), tiltFilter_(25), processing_flag_(true) {
}

#if 0
KSensor::KSensor(JNIEnv& jniEnv, jobject activity) :
        gyroFilter_(KGyroNoiseFilterCapacity), tiltFilter_(25), processing_flag_(true), activity_(activity) {
...
void KSensor::updateQ(KTrackerMessage *msg, glm::vec3& corrected_gyro, glm::quat& q) {
    ...
    if (0.0f != axisAndMagnitude.second) {
        q *= glm::angleAxis(axisAndMagnitude.second * deltaT, axisAndMagnitude.first);
    }
...
glm::vec3 KSensor::applyGyroFilter(const glm::vec3& rawGyro, const float currentTemperature) {
    ...
    if (gyroFilterSize > 0) {
        if (!isnan(sensorTemperature_) && sensorTemperature_ != currentTemperature && 0 != currentTemperature) {
            ...
            bool c2 = glm::length(rawGyro - mean) > 0.037f;
            if (c1 || c2) {
                gyroFilter_.clear();
...
glm::quat KSensor::applyMagnetometerCorrection(glm::quat& orientation, const glm::vec3& accelerometer,
    ...
    if (0 != glm::length(referencePoint_.first)) {
        ...
        if (angleRadians < glm::radians(5.0f)) {
            ...
            correction *= deltaT;
            correctedOrientation = glm::angleAxis(correction, glm::vec3(0.0f, 1.0f, 0.0f)) * orientation;
...
#ifdef DEBUG_SENSOR_RECORDER
static const char * const kFileName = "/sdcard/head_tracking.dat";

bool KSensor::toggleRecording(const FloatData data) {
    if (replaying_) {
        replayFile_.close();
        replaying_ = false;
    }
    ...
    if (recording_) {
        ...
        replayFile_.open(kFileName, std::ios::out | std::ios::trunc | std::ios::binary);

        for (int i = 0; i < FloatDataSize; ++i) {
            replayFile_ << data[i] << " ";
        }
        replayFile_ << std::endl;
    }

    recording_ = replayFile_.is_open();
    return recording_;
}

bool KSensor::toggleReplaying(FloatData data) {
    if (recording_) {
        replayFile_.close();
        recording_ = false;
    }
    ...
    if (replaying_) {
        ...
        replayFile_.open(kFileName, std::ios::in | std::ios::binary);
    ...
    replaying_ = replayFile_.is_open();
    return replaying_;
}
#endif
